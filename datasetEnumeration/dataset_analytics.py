...
maxi_item = 0
answer_item = None # самый часто встречаемый item

maxi_url = 0
answer_url = None # самый часто встречаемый url
...
with open("items_statistics_all_lines") as out_file:
    out_file.write(sorted(counter_all_items.items(), key=lambda item: item[1]).__repr__())

with open("urls_statistics_all_lines") as out_file:
